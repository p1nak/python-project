...
class Target:
    ...
    def draw(self, win):
        ...
        pygame.draw.circle(win, self.SEC_COLOR, (self.x, self.y), int(self.size * 0.8))
        pygame.draw.circle(win, self.COLOR, (self.x, self.y), int(self.size * 0.6))
        pygame.draw.circle(win, self.SEC_COLOR, (self.x, self.y), int(self.size * 0.4))
...
def end_screen(win, elapsed_time, target_pressed, clicks):
    ...
    restart_label = LABEL_FONT.render("Press R to Restart or Q to Quit", 1, "yellow")
    ...
    while True:  
        ...
        while run:
            ...
            for target in targets[:]:
                ...
                if misses >= LIVES:
                    restart = end_screen(win, elapsed_time, target_pressed, clicks)
                    if restart:
                        run = False 
